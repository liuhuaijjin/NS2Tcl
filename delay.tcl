...
proc setQueueEnableDelay { } {
    global arrLink
    foreach i [array names arrLink] {
        [$arrLink($i) queue] enableDelayCheck
    }
}

proc setQueueAddFidPrior { jobId } {
    global arrLink
    foreach i [array names arrLink] {
        [$arrLink($i) queue] addFidPrior $jobId
    }
}
...
proc startJob {job_a jobId ftp_a wtime {numMb 100}} {
    ...
    for {set j 0} {$j < $reducen} {incr j} {
        for {set i 0} {$i < $mp} {incr i} {
			...
			set time [expr $wtime + [avgDistribution]]
			if {1 == $isFlowBased} {
				$ns at $time "centrlCtrl $arrftp($jobId,$i,$j) $CmdaddFlow"
			}
			$ns at $time "setQueueAddFidPrior $jobId"
    ...
    for {set j 0} {$j < $reducen} {incr j} {
        for {set k [expr $mapn - 1]} {$k >= 0} {set k [expr $k - 1]} {
            if {$ftp($jobId,$k,$j,status) == "s" && yes == [$ftp($jobId,$k,$j)   isend]} {
				...
				set time [expr $now + [avgDistribution]]
				if {1 == $isFlowBased} {
					$ns at $time "centrlCtrl $ftp($jobId,$k,$j) $CmdremoveFlow"
				}
                ...
                if { $started < $mapn} {
					...
					set time [expr $now + [avgDistribution]]
					if {1 == $isFlowBased} {
						$ns at $time "centrlCtrl $ftp($jobId,$started,$j) $CmdaddFlow"
					}
...
# trace记录文件，nam动画记录文件
set		f	[open simu/delay.tr w]
set		nf	[open simu/delay.nam w]
# 设置nam记录
#$ns namtrace-all $nf
#$ns trace-all $f
